...
export const UserManagementSection = () => {
  ...
  const handleToggleAffiliate = async (userId: string) => {
    setProcessingUser(userId);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-users`,
        {
          ...
          body: JSON.stringify({ action: 'toggle_affiliate', userId })
        }
      );
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to toggle affiliate');
      }

      toast.success("Status de afiliado atualizado");
      await fetchUsers();
    } catch (error: any) {
                ...
                <TableRow key={userItem.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex gap-2 justify-end flex-wrap">
                      ...
                      <Button
                        variant={userItem.is_affiliate ? "secondary" : "outline"}
                        size="sm"
                        onClick={() => handleToggleAffiliate(userItem.id)}
                        disabled={processingUser === userItem.id}
                      >
                        {processingUser === userItem.id ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
