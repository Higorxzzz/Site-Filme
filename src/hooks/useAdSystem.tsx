...
export const useAdSystem = () => {
  ...
  const watchFiveAds = async (adsWatched: number) => {
    ...
    if (adsWatched >= requiredAds && user) {
      ...
      if (!error) {
        ...
        // Registrar comissão para afiliado (anúncios diários)
        try {
          const { data: settings } = await supabase
            .from('affiliate_settings')
            .select('daily_ads_reward')
            .maybeSingle();
          ...
          if (settings) {
            await supabase.functions.invoke('affiliate-track', {
              body: {
                action: 'conversion',
                userId: user.id,
                conversionType: 'daily_ads',
                amount: settings.daily_ads_reward
              }
            });
          }
        } catch (error) {
          console.error('❌ Erro ao registrar comissão de anúncios:', error);
        }
      } else {
  ...
  const grantAdFreeTime = async (minutes: number) => {
    ...
    if (!error) {
      ...
      // Registrar comissão para afiliado (anúncio por tempo)
      try {
        const { data: settings } = await supabase
          .from('affiliate_settings')
          .select('per_ad_reward')
          .maybeSingle();
        ...
        if (settings) {
          await supabase.functions.invoke('affiliate-track', {
            body: {
              action: 'conversion',
              userId: user.id,
              conversionType: 'per_ad',
              amount: settings.per_ad_reward
            }
          });
        }
      } catch (error) {
        console.error('❌ Erro ao registrar comissão de anúncio por tempo:', error);
      }
    }
